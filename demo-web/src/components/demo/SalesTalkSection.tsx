    ...
    <section className="mx-auto mt-16 max-w-4xl rounded-lg border border-border bg-white p-8 shadow-sm">
      ...
      <div className="mt-6 rounded-md border border-border bg-blue-50 p-4">
        ...
        <p className="mt-2 text-sm text-text">
          お客様の価値観（{(topRecommendation as any).decision_style || "バランス型"}）に合わせて、
          {(topRecommendation as any).decision_style === "Maximizer" && "詳細な比較データと豊富な選択肢を提示"}
          {(topRecommendation as any).decision_style === "Satisficer" && "必要十分な情報で迅速な意思決定をサポート"}
          {(topRecommendation as any).decision_style === "Authority-driven" && "専門家の評価と実績を重視した説明"}
          {(topRecommendation as any).decision_style === "Delegator" && "他のお客様の声と専門家の推奨を中心に"}
          {(topRecommendation as any).decision_style === "Intuitive" && "実際の体験と感覚を大切にした提案"}
          {(topRecommendation as any).decision_style === "Impulsive" && "今だけの特典と即決のメリットを強調"}
          {!(topRecommendation as any).decision_style && "お客様に寄り添った丁寧な説明"}
          することで、納得感のある提案になります。
        </p>
      </div>
    </section>
...
function generateSalesTalk(recommendation: Recommendation): SalesTalkPoint[] {
  ...
  points.push({
    title: "お客様のご要望の確認",
    talk: `お客様は${(recommendation as any).matched_needs?.slice(0, 2).join("と") || "快適性と安全性"}を重視されているとのことですね。${recommendation.model}はまさにそのニーズにお応えできる一台です。`,
  });
  ...
  points.push({
    title: "実際の使用イメージ",
    talk: `例えば、週末のご家族でのお出かけや日々の通勤など、様々なシーンで快適にご利用いただけます。特に${recommendation.model}の${(recommendation as any).features?.[0]?.name || "先進機能"}は、実際にお使いいただくとその良さを実感していただけると思います。`,
  });
